...
import { lateNightCrossings } from "@/data/late-night-crossings";

export default function LateNightCrossingsPage() {
  const v = lateNightCrossings;
    ...
    <main className="min-h-screen" style={{ background: "#0f0d0a" }}>
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="relative w-full mb-10 gold-border overflow-hidden" style={{ aspectRatio: "16/9" }}>
          <Image src={v.imageLarge} alt={v.name} fill style={{ objectFit: "cover", objectPosition: "center top" }} priority />
        </div>
        ...
        {/* Owner */}
        <div className="mt-8 mb-10">
          <h2 className="text-xs uppercase tracking-widest mb-4" style={{ color: "var(--gold)", letterSpacing: "0.2em" }}>The Innkeeper</h2>
        ...
        <div className="mt-8">
          ...
          {dmOpen && (
            <div className="p-6 space-y-5" style={{ background: "rgba(139,26,26,0.06)", border: "1px solid rgba(139,26,26,0.3)", borderTop: "none" }}>
              ...
              <div className="gold-divider" style={{ background: "linear-gradient(to right, transparent, rgba(139,26,26,0.5), transparent)" }} />
              <div>
                <h4 className="text-xs uppercase tracking-widest mb-2" style={{ color: "#e88080" }}>DM Notes</h4>
              ...
              <div className="gold-divider" style={{ background: "linear-gradient(to right, transparent, rgba(139,26,26,0.5), transparent)" }} />
              <div>
                <h4 className="text-xs uppercase tracking-widest mb-3" style={{ color: "#e88080" }}>Private Arrangements</h4>
        ...
        {/* Menu */}
        <div className="mt-8">
          <h2 className="text-xs uppercase tracking-widest mb-8" style={{ color: "var(--gold)", letterSpacing: "0.2em" }}>The Menu</h2>

          {/* Hot Meals */}
          <h3 className="text-xs uppercase tracking-widest mb-4" style={{ color: "var(--gold)", opacity: 0.6, letterSpacing: "0.15em" }}>Hot Meals</h3>
          <div className="space-y-3 mb-10">
            {v.menu.hotMeals.map((item, i) => (
          ...
          {/* Breakfast */}
          <h3 className="text-xs uppercase tracking-widest mb-4" style={{ color: "var(--gold)", opacity: 0.6, letterSpacing: "0.15em" }}>Breakfast</h3>
          <div className="space-y-3 mb-10">
            {v.menu.breakfast.map((item, i) => (
              <div key={i} className="px-5 py-4" style={{ background: i % 2 === 0 ? "rgba(201,168,76,0.03)" : "transparent", border: "1px solid rgba(201,168,76,0.12)" }}>
                <div className="flex items-start justify-between gap-4 mb-1">
                  <p className="text-sm font-bold" style={{ color: "var(--gold-light)" }}>{item.name}</p>
                  <p className="text-sm font-bold whitespace-nowrap" style={{ color: "var(--gold)" }}>{item.price}</p>
          ...
          {/* Drinks */}
          <h3 className="text-xs uppercase tracking-widest mb-4" style={{ color: "var(--gold)", opacity: 0.6, letterSpacing: "0.15em" }}>Drinks</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-10">
            {v.menu.drinks.map((item, i) => (
              <div key={i} className="px-4 py-3 flex items-center justify-between" style={{ border: "1px solid rgba(201,168,76,0.12)", background: "rgba(201,168,76,0.02)" }}>
                <p className="text-sm" style={{ color: "var(--parchment)", opacity: 0.85 }}>{item.name}</p>
                <p className="text-sm font-bold ml-3" style={{ color: "var(--gold)" }}>{item.price}</p>
              </div>
            ))}
          </div>

          {/* Rooms */}
          <h3 className="text-xs uppercase tracking-widest mb-4" style={{ color: "var(--gold)", opacity: 0.6, letterSpacing: "0.15em" }}>Rooms</h3>
          <div className="space-y-3">
            {v.menu.rooms.map((item, i) => (
              <div key={i} className="px-5 py-4" style={{ background: i % 2 === 0 ? "rgba(201,168,76,0.03)" : "transparent", border: "1px solid rgba(201,168,76,0.12)" }}>
                <div className="flex items-start justify-between gap-4 mb-1">
                  <p className="text-sm font-bold" style={{ color: "var(--gold-light)" }}>{item.name}</p>
                  <p className="text-sm font-bold whitespace-nowrap" style={{ color: "var(--gold)" }}>{item.price}</p>
