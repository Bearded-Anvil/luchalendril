...
import { clanksSmithyData } from "@/data/clanks-smithy";

export default function ClanksSmithyPage() {
  const v = clanksSmithyData;
  ...
  return (
    <main className="min-h-screen" style={{ background: "#0f0d0a" }}>
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        {/* Owner */}
        <div className="mt-8 mb-10">
          <h2 className="text-xs uppercase tracking-widest mb-4" style={{ color: "var(--gold)", letterSpacing: "0.2em" }}>The Smith</h2>
        ...
        <div className="mt-8">
          ...
          {dmOpen && (
            <div className="p-6 space-y-5" style={{ background: "rgba(139,26,26,0.06)", border: "1px solid rgba(139,26,26,0.3)", borderTop: "none" }}>
              ...
              <div className="gold-divider" style={{ background: "linear-gradient(to right, transparent, rgba(139,26,26,0.5), transparent)" }} />
              <div>
                <h4 className="text-xs uppercase tracking-widest mb-3" style={{ color: "#e88080" }}>Custom Commissions</h4>
              ...
              >
                <div className="flex items-start justify-between gap-4 mb-2">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-sm font-bold" style={{ color: "var(--gold)" }}>{item.price}</p>
                    {"stock" in item && item.stock && (
                      <p className="text-xs mt-0.5" style={{ color: "var(--parchment)", opacity: 0.45 }}>{item.stock}</p>
                    )}
                  </div>
                </div>
                ...
                {"mechanical" in item && item.mechanical && (
                  <p className="text-xs leading-relaxed mb-2 px-3 py-2" style={{ color: "#e8c97a", opacity: 0.85, background: "rgba(201,168,76,0.06)", borderLeft: "2px solid rgba(201,168,76,0.3)" }}>
                    ⚙ {item.mechanical}
                  </p>
                )}
                {"quote" in item && item.quote && (
                  <p className="text-xs leading-relaxed italic mt-2" style={{ color: "var(--parchment)", opacity: 0.5 }}>
