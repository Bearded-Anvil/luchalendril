...
import { copperPotSteam } from "@/data/copper-pot-steam";

export default function CopperPotSteamPage() {
  const v = copperPotSteam;
    ...
    <main className="min-h-screen" style={{ background: "#0f0d0a" }}>
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="relative w-full mb-10 gold-border overflow-hidden" style={{ aspectRatio: "16/9", maxHeight: "540px" }}>
          <Image src={v.image} alt={v.name} fill style={{ objectFit: "cover", objectPosition: "right center" }} priority />
        </div>
            ...
            {v.services.map((service, i) => (
              <div key={i} className="px-5 py-4" style={{ border: "1px solid rgba(201,168,76,0.2)", background: "rgba(201,168,76,0.03)" }}>
                <div className="flex items-start justify-between gap-4 mb-1">
                  ...
                  <p className="text-sm shrink-0" style={{ color: "var(--gold)", opacity: 0.8 }}>{service.cost}</p>
                </div>
