...
import { tinkerBox } from "@/data/tinker-box";

type InventoryItem = typeof tinkerBox.inventory[0];
...
function InventoryCard({ item }: { item: InventoryItem }) {
  return (
    <div className="px-5 py-4" style={{
      ...
      <p className="text-xs mb-1" style={{ color: "var(--gold)", opacity: 0.45 }}>{item.stock}</p>
      ...
      {item.dmNote && (
        <p className="text-xs leading-relaxed mt-1" style={{ color: "#e88080", opacity: 0.7 }}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        <Breadcrumb
          crumbs={[
            ...
            { label: "Southeast Beacon Street", href: "/beacon/southeast-beacon-street" },
            { label: v.name },
          ]}
        />

        <div className="mb-8">
          <p className="text-xs uppercase tracking-widest mb-1" style={{ color: "var(--gold)", letterSpacing: "0.25em" }}>Southeast Beacon Street</p>
        ...
        <div className="mt-8 mb-10 flex flex-col sm:flex-row gap-4">
          {v.signs.map((sign, i) => (
            <div key={i} className="flex-1 px-5 py-4 text-center" style={{
              background: "rgba(201,168,76,0.04)",
              border: "1px solid rgba(201,168,76,0.18)",
            }}>
              <p className="text-xs italic leading-relaxed" style={{ color: "var(--parchment)", opacity: 0.6 }}>{sign}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 mb-10">
          ...
          <div className="px-6 py-5" style={{ background: "rgba(201,168,76,0.05)", border: "1px solid rgba(201,168,76,0.25)" }}>
            <div className="flex items-baseline gap-3 mb-1">
              ...
              <p className="text-xs" style={{ color: "var(--gold)", opacity: 0.5 }}>{v.owner.race} · {v.owner.age}</p>
            </div>
        ...
        <div className="mt-8 mb-10">
          ...
          <p className="text-xs italic mb-6" style={{ color: "var(--parchment)", opacity: 0.4 }}>Gadgets, tools, and things that probably shouldn&rsquo;t exist.</p>
        ...
        <div className="mt-8 mb-6">
          ...
          {dmOpen && (
            <div className="dm-private p-6 space-y-6" style={{ borderTop: "none" }}>
              <div>
                <h4 className="text-xs uppercase tracking-widest mb-2" style={{ color: "#e88080", letterSpacing: "0.15em" }}>Fears</h4>
                <p className="text-sm leading-relaxed" style={{ color: "var(--parchment)", opacity: 0.85 }}>{v.dmPrivate.fears}</p>
              </div>
              <div style={{ borderTop: "1px solid rgba(139,26,26,0.25)" }} />
              <div>
                <h4 className="text-xs uppercase tracking-widest mb-2" style={{ color: "#e88080", letterSpacing: "0.15em" }}>Goals</h4>
                <p className="text-sm leading-relaxed" style={{ color: "var(--parchment)", opacity: 0.85 }}>{v.dmPrivate.goals}</p>
              </div>
              <div style={{ borderTop: "1px solid rgba(139,26,26,0.25)" }} />
              <div>
                <h4 className="text-xs uppercase tracking-widest mb-2" style={{ color: "#e88080", letterSpacing: "0.15em" }}>Quest Hook — The Commission</h4>
                {v.dmPrivate.questHook.split("\n\n").map((paragraph, i) => (
                  <p key={i} className="text-sm leading-relaxed mb-3 last:mb-0" style={{ color: "var(--parchment)", opacity: 0.85 }}>{paragraph}</p>
                ))}
              </div>
            </div>
          )}
