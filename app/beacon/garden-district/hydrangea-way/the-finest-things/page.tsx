...
import { theFinestThings } from "@/data/the-finest-things";

type ServiceItem = typeof theFinestThings.services[0]["items"][0];
    ...
    <div className="px-5 py-4" style={{
      ...
      <div className="flex items-start justify-between gap-4 mb-2">
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          ...
          {item.commission && (
            <p className="text-xs whitespace-nowrap" style={{ color: "var(--parchment)", opacity: 0.35 }}>{item.commission} commission</p>
          )}
        </div>
      </div>
...
export default function TheFinestThingsPage() {
  const [dmOpen, setDmOpen] = useState(false);
  const v = theFinestThings;
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="mt-8 mb-10">
          ...
          {v.sceneDescription.split("\n\n").map((paragraph, i, arr) => {
            const isSign = paragraph.startsWith("All jobs are commissions");
            if (isSign) {
        ...
        {/* Services */}
        <div className="mt-8 mb-10">
          <h2 className="text-xs uppercase tracking-widest mb-2" style={{ color: "var(--gold)", letterSpacing: "0.2em" }}>Menu of Services</h2>
          <p className="text-xs italic mb-8" style={{ color: "var(--parchment)", opacity: 0.4 }}>{v.dmNote}</p>

          <div className="space-y-10">
            {v.services.map((section, si) => (
              <div key={si}>
                <h3 className="text-xs uppercase tracking-widest mb-4 pb-2" style={{
                  color: "var(--gold)",
                  opacity: 0.6,
                  letterSpacing: "0.2em",
          ...
          {dmOpen && (
            <div className="px-6 py-5 space-y-5" style={{ background: "rgba(139,26,26,0.08)", border: "1px solid rgba(139,26,26,0.25)" }}>
              ...
              <div>
                ...
                <p className="text-sm leading-relaxed" style={{ color: "var(--parchment)", opacity: 0.75 }}>{v.owner.dmPrivate.note}</p>
              </div>
