...
import { roddikssRangedGoods } from "@/data/roddiks-ranged-goods";

export default function RoddiksRangedGoodsPage() {
  const v = roddikssRangedGoods;
            ...
            <div className="space-y-2">
              {section.items.map((item, i) => (
                <div key={i} className="px-5 py-3" style={{
                  ...
                  {"rules" in item && item.rules && (
                    <p className="text-xs leading-relaxed mt-2" style={{ color: "var(--gold)", opacity: 0.65 }}>{item.rules}</p>
                  )}
                </div>
              ))}
            </div>
