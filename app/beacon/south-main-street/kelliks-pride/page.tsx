...
import { kelliksPride } from "@/data/kelliks-pride";

export default function KelliksPridePage() {
  const v = kelliksPride;
  ...
  return (
    <main className="min-h-screen" style={{ background: "#0f0d0a" }}>
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        {/* Inventory */}
        <div className="mt-8">
          <h2 className="text-xs uppercase tracking-widest mb-2" style={{ color: "var(--gold)", letterSpacing: "0.2em" }}>Jewelry & Enchantments</h2>
          <p className="text-xs italic mb-6" style={{ color: "var(--parchment)", opacity: 0.45 }}>All items are enchanted. All effects are real.</p>
        ...
        <div className="mt-8">
          ...
          {dmOpen && (
            <div className="dm-private p-6 mt-0 space-y-5" style={{ borderTop: "none" }}>
              ...
              <div>
                <h4 className="text-xs uppercase tracking-widest mb-2" style={{ color: "#e88080" }}>The Cat</h4>
                <p className="text-sm leading-relaxed" style={{ color: "var(--parchment)", opacity: 0.9 }}>{v.dmPrivate.catNote}</p>
              </div>
              ...
              {/* Secret Items */}
              <div>
                <h4 className="text-xs uppercase tracking-widest mb-1" style={{ color: "#e88080" }}>Secret Inventory</h4>
                <p className="text-xs italic mb-4" style={{ color: "var(--parchment)", opacity: 0.5 }}>
                  Offered only to customers who have spent coin, proven discretion, or asked for something with real edge to it.
                </p>
                <div className="space-y-5">
                  {v.dmPrivate.secretItems.map((item, i) => (
                    <div key={i} className="pl-4" style={{ borderLeft: "2px solid rgba(139,26,26,0.4)" }}>
                      ...
                      <p className="text-xs italic mb-2" style={{ color: "var(--parchment)", opacity: 0.5 }}>{item.rarity}</p>
                      <p className="text-xs leading-relaxed mb-2" style={{ color: "var(--parchment)", opacity: 0.85 }}>{item.description}</p>
                      <p className="text-xs mb-2" style={{ color: "var(--gold)", opacity: 0.7 }}><span className="font-bold">Effect: </span>{item.effect}</p>
                      <p className="text-xs mb-2" style={{ color: "var(--parchment)", opacity: 0.65 }}><span className="font-bold">Drawback: </span>{item.drawback}</p>
                      <p className="text-xs italic" style={{ color: "#e88080", opacity: 0.75 }}>{item.note}</p>
                    </div>
                  ))}
                </div>
              </div>
